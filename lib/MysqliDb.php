...
class MysqliDb
{
    ...
    /**
     * For function JSON_CONTAINS written by Saintly2k
     */
    protected $_jsonContains = array();
    ...
    public function jsonContains($whereProp, $whereValue = "DBNULL", $cond = "", $third = "$")
    {
        $this->_jsonContains[] = array($whereProp, $whereValue, $cond, $third);
        return $this;
    }
    ...
    protected function _buildQuery($numRows = null, $tableData = null)
    {
        ...
        $this->_buildJsonContains($this->_jsonContains);
    ...
    protected function _buildCondition($operator, &$conditions)
    {
        ...
        if ($operator == "WHERE" && empty($this->_jsonContains)) {
    ...
    protected function _buildJsonContains($values)
    {
        if (empty($values)) {
            return;
        }

        //Prepare the JSON_CONTAINS portion of the query
        $c = 1;
        foreach ($values as $value) {
            $this->_query .= " {$value[2]} WHERE JSON_CONTAINS(`{$value[0]}`,'{$value[1]}','{$value[3]}')" . ($c != 1 ? " AND" : "");
            $c++;
        }
        if (!empty($this->_where)) $this->_query .= " AND";
    }
